...
- `init-command`:
  - Provide the full command string to be executed by the Action. The default is `kam init . -t kam -f`. Note: `-t` accepts a short template name (e.g., `kam`, `meta`, `ak3`) which will be resolved to `<id>_template` (e.g., `kam` -> `kam_template`). You may also provide a full built-in template ID (e.g., `kam_template`) or a local path/archive (e.g., `./template-folder` or `https://.../template.zip`). You can still use `kam init .` or add additional arguments.
