...
- `init-command`：
  - 输入完整的命令行字符串，Action 会在 shell 中执行。例如默认 `kam init . -t kam -f`，也可以改为 `kam init .` 或带其他参数。
  - 注意：`-t`（或 `--template`）支持短 id（例如 `-t kam`、`-t meta`、`-t ak3`），会自动解析为 `<id>_template`（例如 `-t kam` -> `kam_template`）。如果传入的是完整模板 id（如 `kam_template`）或路径/归档（如 `./template-folder` 或 `https://.../template.zip`），将按原样使用。
